...
Shader "Unlit/Shader0" // shader name  - shader1 under shader category - Unlit
{
    ...
    Properties
    {
        ...
        _Power("Power", float) = 1 //with a specified range.
        _SecondPower("Second Power", float) = 0.5
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float3 color = dot(i.viewDirection, i.world_normal);
                fixed4 FinalColor = fixed4(color, 1);
                FinalColor = saturate(FinalColor);
                //(1,1,1,1) 
                fixed4 OneMinus = fixed4(1,1,1,1) - FinalColor;
                fixed4 result = pow(OneMinus,_Power);
                fixed4 final = saturate(pow(fixed4(1,1,1,1)-result,_SecondPower));
                fixed4 _mainTex = tex2D(_MainTex, fixed4(1,1,0,0));
                return _mainTex*final;
            }
            ENDCG
        }
    }
